...
def main():
    ...
    # 各 label 所有 folds 的 RF 模型資訊（用來算 Avg folds importance & SHAP）
    ...
    for train_idx, test_idx in mskf.split(X, Y_multi):
        ...
        for label in label_names:
            ...
            # 記錄每個 fold 的 RF 模型，用來做 Avg folds 的 importance / SHAP
            if "RF" in results:
                r_rf = results["RF"]
    ...
    rf_importance_xlsx = os.path.join(run_dir, "baseline_RF_Importances_AvgFolds.xlsx")
    with pd.ExcelWriter(rf_importance_xlsx) as writer:
        for label in label_names:
            ...
            top_df = imp_df.head(top_k).iloc[::-1]  # 反轉讓重要性大的在上面
    ...
    print(
        "\n✅ 完成！只保留每個標籤的整體最佳模型，並輸出各 label 的 RF importance / SHAP / permutation (Avg folds)，檔案位置:",
        run_dir,
    )
    print("=" * 70)
