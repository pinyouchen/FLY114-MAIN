...
class DataProcessorBaseline:
    ...
    - 特徵：HRV + 人口學 + 心理量表
      ...
      Demo: Age, Sex, BMI
      Psych: phq15, haq21, cabah, bdi, bai
    - 工程特徵：HRV_Mean, LF_HF_Ratio
    - 分類任務：疾病 vs 真健康人
      疾病=1：SSD/MDD/Panic/GAD 各自的1
      健康=0：Health==1 且 SSD,MDD,Panic,GAD 全部為0
    """
    ...
    def prepare_features_and_labels(self):
        ...
        # 僅把 4 個疾病 label 存進 y_dict，Health 留在 df 中用來當 filter
...
def main():
    print("\n" + "="*70)
    print("🏁 Baseline：HRV+Demo+Psych，疾病 vs 真健康人（只保留每個 label 的整體最佳模型）")
    print("="*70)
    ...
    label_names = processor.label_names          # 只包含 SSD, MDD, Panic, GAD
    df_all = processor.df                        # 含 Health 與所有 label 原始欄位

    # Multi-label KFold 仍用四個疾病欄位去做 stratify
    ...
    ax.set_ylabel('F1-score'); ax.set_title('F1-score by Label (Disease vs True Health)')
